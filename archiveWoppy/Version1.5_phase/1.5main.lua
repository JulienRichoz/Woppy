...
--Description v. 1.5: Gestion des lancement de sorts d'Immerseus
--Améliorations: changement variables en globales car locales ne s'enregistrent pas correctement à travers les frames
--				Meilleur gestion changement de phases
...
--A FAIRE: CAPTER MESSAGE POUR CHANGER DE PHASE avec truc semblable a CHAT_MSG_RAID_BOSS_EMOTES
--PROBLEME QUAND 2 STACK CAR L'EVENT AURA APPLIED NE SE DECLENCHE PAS
			...
			for i=1,5 do 
				...
				if(spellID==idDebuff) then 
					if(unit~=playerUnit) then
						return RaidNotice_AddMessage(RaidWarningFrame, "Reprend la menace!", ChatTypeInfo["RAID_WARNING"]);
					end
				end
			end
		end
...
local function StratImmerseus()
	...
	immerseusFrame:SetScript("OnEvent", function(self, event, ...)
		if(event=="COMBAT_LOG_EVENT_UNFILTERED") then 
			...
			if(woppy_vieBoss<5 and woppy_flagVieBoss==0) then 
			...
			--Passage en P2
			if(woppy_vieBoss<=0.5 and woppy_flagVieBoss==1) then
			...
			if (event == "SPELL_AURA_APPLIED") then
				...
				if(spellName == explosionCaustique) then
					...
					--On verifie si l'on possède le debuff
					for i=1,5 do 
						local name, rank, icon, count, dispelType, duration, expires, caster, isStealable, shouldConsolidate, spellID= UnitDebuff("player",i);
						print("Stack avant le if: "..count);
						if(spellID==143436) then --explosion caustique
							print("Stack dans le if: "..count);
							print("Vous avez "..count.." stack de "..name);
							print(name, count, spellID, duration, expires);
							if(count>1) then
							...
							if(woppy_specRole=="HEALER" or woppy_specRole=="DAMAGER") then 
								...
								print("Dans le second if: stack: "..count);
							end
						end	
						--si on ne l'a pas et que nous sommes tanks on vérifie qui du raid possède le debuff (reprise aggro)
			...
			end--fin event aura applied
			if(event=="SPELL_AURA_REFRESH") then
				if(spellName==explosionCaustique) then
					for i=1, 5 do 
						local name, _, _, count, _, duration, _, _, _, _, spellID =UnitDebuff("player", i);
						print("Vous avez "..count.." stacks!")
						if(woppy_specRole=="TANK" and spellName == nil) then
							ScanForDebuff(143436) --Explosion caustique
						end
					end
		...
		-- if(event == "CHAT_MSG_RAID_BOSS_EMOTE") then 
			-- print(message);
			-- print(arg1);
			-- if(message=="spell:143469") then --reformation
				-- print("reformation-P1");
			-- end
			-- if(message=="spell:143020") then --scission
				-- print("scission-P2");
			-- end
		-- end
