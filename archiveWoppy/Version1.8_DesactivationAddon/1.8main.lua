...
--Description v. 1.8: désactivation de l'addon si le boss est mort
...
--		v. 1.7: Meilleure gestion des frames
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
...
woppy_startTime=nil; --Timer
woppy_unitImmerseus=nil; --focus IMMERSEUS
woppy_vieBoss=nil; --Vie BOSS
woppy_corruptionBoss=nil; --Energie du boss
woppy_disableAddon=0; --Désactiver l'addon 
...
--3.5 Fonction désactiver addon
local bossDead = function(bossNumber)
	local _, _, _, _, _, _, _, _, _, _, _, whichDied = GetSavedInstanceInfo();
	if(whichDied==bossNumber) then 
		woppy_disableAddon=1;
		DisableAddon("Woppy");
	else 
		woppy_disableAddon=0;
	end
end
...
areaFrame:SetScript("OnEvent", function(self, event, ...)
	...
	if(raidID==1136) then	--Si 	c'est le raide siège d'orgrimmar..
		...
		if(woppy_disableAddon==0) then 
			...
			CheckCombat();
		elseif(woppy_disableAddon==1) then
			DisableAddon("Woppy");
		end
	end
end)
